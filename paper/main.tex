...
This project is organized around four falsifiable research questions.
...
\begin{description}
  ...
  \item[RQ4: Generality.] Which interfaces can be shared by Touhou 1--5, and
...
only preliminarily.  RQ4 remains a design target: only TH05 is validated.
...
TH05 starts at stage 1 with three lives, three bombs, character 2, rank 0, and
power 0.  The environment advances at a 36 ms action interval.  Evaluations are
stochastic and paired by PyTorch sampling seed.  The game itself is not yet
...
\begin{itemize}
  \item The final test has only four stochastic seeds and 900 steps per seed.
  The paired standard error is large and no stage clear occurs.
  ...
  \item All learning evidence comes from one TH05 stage configuration.  It does
  not establish transfer to other stages, characters, ranks, or TH01--04.
...
Add normal-enemy tokens and analytic time-to-collision features while keeping
the observation below 2 KiB.  Compare against the 273-float schema with matched
training transitions and validation/test splits.

\paragraph{H2: curriculum and generalization.}
Randomize stage, phase, character, rank, power, lives, and bombs.  Use stage
completion and resource-normalized completion as primary metrics.  Hold out
entire configurations, not only sampling seeds.
...
\paragraph{H5: old-five-games interface.}
...
live parser/action tests pass; shared code alone is not evidence of support.
