...
highest difficulty; only TH05 is currently validated.
...
\begin{itemize}
  ...
  benchmark.
  \item API steps are paced by a wall-clock interval rather than synchronized
  to an observed native-frame transition.  Setting that interval to zero makes
  the policy loop outrun emulation and cannot be interpreted as additional
...
produces zero no-miss clears.  The persistent gap motivates hazard attribution,
special-projectile and deathbomb semantics, and constrained survival learning
rather than a claim of mastery.  The main scientific task is to turn this systems
