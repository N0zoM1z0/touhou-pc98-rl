...
only preliminarily.  RQ4 has an underpowered comparative result.  RQ5 is
motivated by the observed completion/no-miss gap but not yet tested.  RQ6 is
deliberately deferred until the agent can complete TH05 on its highest
...
\paragraph{H4: sparse-miss auxiliary credit.}
For each on-policy transition, predict whether a miss occurs within 15 and 30
future actions.  Mask samples whose horizon crosses an unfinished rollout
boundary, stop gradients through labels, and expose no future information at
inference.  Compare PPO and PPO plus the shared-encoder auxiliary loss at
matched transitions.  The hypothesis is rejected if no-miss completion does
not improve or ordinary completion regresses.
...
improvements in short-phase completion and survival.  The persistent gap
between completion and no-miss completion motivates an auxiliary-credit
ablation rather than a claim of mastery.  The main scientific task is to turn
this systems baseline into a pre-registered multi-configuration study with
